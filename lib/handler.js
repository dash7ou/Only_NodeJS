        ...
        try {
            if (data.method === 'get') {
                const templateData = {
                    ...
                    'head.description': "Maritime Montering is Europe’s trusted & preferred supplier of ship interior for marine and offshore Ship Accommodation projects. We provide marine interior outfitting, both full turnkey as well tailor-made Scope of work, to achieve the best possible solution and to meet customer needs.",
                    'body.class': 'index'
                }
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (data.method === 'get') {
                ...
                let str = await getTemplate('accountCreate', templateData);
                str = await addUnibersalTemplates(str, templateData);
                resolve([200, str, 'html'])
                // resolve([405, undefined, 'html'])
            }
...
// create new session
exports.sessionCreate = data => {
    return new Promise(async (resolve, reject) => {
        try {
            if (data.method === "get") {
                const templateData = {
                    'head.title': "Login to ur account",
                    'head.description': "Please enter ur phone number and password to access ur account.",
                    'body.class': 'sessionCreate'
                }

                let str = await getTemplate('sessionCreate', templateData);
                str = await addUnibersalTemplates(str, templateData);
                resolve([200, str, 'html'])
            }
        } catch (err) {
...
handlers._users.get = async (data, resolve, reject) => {
    ...
    if (!phone) {
        resolve([400, {
            error: "Phone not found to find user"
        }])
    }
...
handlers._users.post = async (data, resolve, reject) => {
    ...
    toAgreement = toAgreement ? true : false
...
handlers._users.put = async (data, resolve, reject) => {
    ...
    try {
        ...
        if (!firstName && !lastName && !password) {
            resolve([400, {
                error: "No field found to update it."
            }])
        }
...
handlers._tokens.post = async (data, resolve, reject) => {
    ...
    if (!phone || !password) {
        resolve([400, {
            error: "there are missing required data"
        }])
    }
...
handlers._checks.post = async (data, resolve, reject) => {
    ...
    try {
        ...
        if (userChecks.length >= maxChecks) {
            resolve([400, {
                error: "the user has already the max number of checks"
            }])
        }
...
handlers._checks.put = async (data, resolve, reject) => {
    ...
    if (!protocal && !url && !method && !sucessCode && !timeoutSeconds) {
        resolve([400, {
            error: "Missing field to update it!"
        }])
    }
