const { hash, createRandomString, getTemplate, addUnibersalTemplates } = require("./helper")
        ...
        try {
            if (data.method === "get") {
                ...
                if (!assetName) {
                    ...
                    const allowExt = [".css", ".png", ".jpg", ".ico"];
                    let contentType = "plain";
                    const arrayOfName = assetNameData.split(".")
                    if (allowExt.includes(`.${arrayOfName[arrayOfName.length - 1]}`)) {
                        contentType = arrayOfName[arrayOfName.length - 1]
                    } else {
                        return resolve([404]);
                    }
                } else {
